...
async function createTable() {
  ...
  await pgPool.query("CREATE TABLE IF NOT EXISTS public.heat_data (gid SERIAL PRIMARY KEY, weight NUMERIC, geom geometry(POINT,4326));")
...
async function importData(points) {
  ...
  points.map(function (point) {
    promises.push(pgPool.query("INSERT INTO heat_data (weight, geom) VALUES("
      + point._source.weight + ", "
...
async function reindex() {
  await pgPool.query("REINDEX TABLE heat_data")
